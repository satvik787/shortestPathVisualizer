...
export class BFS extends Visualizer{
    constructor(ctx:CanvasRenderingContext2D,mat:Array<Array<Array<number>>>,cellSize:number){
        super(ctx,mat,cellSize);
    }
    calculateShortestPath(sr: number, sc: number,dr:number,dc:number):Array<Array<number>>{
        ...
        const C = [0,0,1,-1];
        visited[sr][sc] = 1;
        const q = new Queue<Array<number>>();
        q.enqueue([sr,sc]);
        let delay = 10;
        while(q.size > 0){
            const [r,c] = q.dequeue();
            if(this.matrix[r][c][2] === CELL_STATE.destination)break;
            for(let i = 0;i < 4;i++){
                const nr = R[i] + r;
                const nc = C[i] + c;
...
export class DFS extends Visualizer{
    ...
    private rec(r:number,c:number,delay:number,visited:Array<Array<number>>){
